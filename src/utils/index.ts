class Utils {
   ...
   */
  static getNameInitial(name: { match: (arg0: RegExp) => never[] }): string {
   ...
   */
  static getColorContrast(hex: any): string {
    if (!hex) {
      return "dark";
    }
    const threshold = 130;
   ...
   */
  static shadeColor(color: any, percent: any): any {
   ...
   */
  static getSignNum(number: number, positive: any, negative: any): any {
    if (number > 0) {
      return positive;
    }
    if (number < 0) {
   ...
   */
  static antdTableSorter(a: any, b: any, key: string | any): any {
